...
# --- DFS (frontend opcional, endpoint /algorithms/dfs) ---
def run_dfs(graph: ZoneGraph, start: str) -> Dict:
    # Búsqueda en profundidad sobre el grafo de zonas (explora vecinos más similares primero)
    visited: List[str] = []  # lista de nodos en orden de visita
    stack = [start]  # pila LIFO para DFS
    seen = set()  # evita revisitar nodos
    while stack:
        node = stack.pop()  # toma último
        if node in seen:
            continue  # salta si ya fue visto
        seen.add(node)  # marca como visitado
        visited.append(node)  # agrega al recorrido
        neighbors = sorted(graph.adjacency.get(node, []), key=lambda x: x[1])  # vecinos ordenados por peso
        stack.extend([n for n, _ in neighbors])  # apila vecinos

    return {"order": visited, "metrics": _cluster_metrics(graph, visited)}


# --- BFS (frontend opcional, endpoint /algorithms/bfs) ---
def run_bfs(graph: ZoneGraph, start: str) -> Dict:
    # Búsqueda en anchura por niveles para zonas conectadas por similitud
    queue: deque[str] = deque([start])  # cola FIFO
    seen = {start}  # conjunto de visitados
    levels: List[List[str]] = []  # niveles resultantes
    while queue:
        level_size = len(queue)  # nodos en este nivel
        level_nodes: List[str] = []  # lista nivel actual
        for _ in range(level_size):
            node = queue.popleft()  # saca de la cola
            level_nodes.append(node)  # agrega al nivel
            for neigh, _ in sorted(graph.adjacency.get(node, []), key=lambda x: x[1]):  # vecinos ordenados
                if neigh not in seen:
                    seen.add(neigh)  # marca
                    queue.append(neigh)  # encola para siguiente nivel
        levels.append(level_nodes)  # guarda nivel

    flattened = [n for lvl in levels for n in lvl]
    return {"levels": levels, "metrics": _cluster_metrics(graph, flattened)}
...
def run_floyd_warshall(graph: ZoneGraph) -> Dict:
    ...
    return {"nodes": nodes, "matrix": dist}
