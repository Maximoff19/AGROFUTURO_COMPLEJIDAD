...
@app.get("/algorithms/dfs")
async def dfs(start: Optional[str] = Query(None, description="Distrito inicial")):
    start_node = start or _default_start_node()
    if not start_node:
        raise HTTPException(status_code=400, detail="No hay distritos para explorar")
    graph = app.state.graph
    if start_node not in graph.nodes:
        raise HTTPException(status_code=404, detail=f"No se encontró el distrito {start_node}")

    result = await asyncio.to_thread(algorithms.run_dfs, graph, start_node)
    return {"start": start_node, **result}


@app.get("/algorithms/bfs")
async def bfs(start: Optional[str] = Query(None, description="Distrito inicial")):
    start_node = start or _default_start_node()
    if not start_node:
        raise HTTPException(status_code=400, detail="No hay distritos para explorar")
    graph = app.state.graph
    if start_node not in graph.nodes:
        raise HTTPException(status_code=404, detail=f"No se encontró el distrito {start_node}")
    result = await asyncio.to_thread(algorithms.run_bfs, graph, start_node)
    return {"start": start_node, **result}
